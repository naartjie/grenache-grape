...
const { inspect: i, promisify } = require('node:util')
...
class Grape extends Events {
  ...
  constructor (conf) {
    ...
    this.lookupAsync = promisify(this.lookup)
  ...
  async announce (val, port, cb) {
    port = port || this.conf.dht_port
    cb = cb ?? noop
    ...
    const refresh = async () => {
      for (const service of Object.keys(this.services)) {
        const res = await this.lookupAsync(service)
        if (res.length === 0) {
          delete this.services[service]
        } else {
          this.services[service] = res
        }
      }

      console.clear()
      console.log('Grenache Services:')
      console.log(Object.keys(this.services).map(s => `${s} => ${i(this.services[s])}`).join('\n'))
      console.log()
    }

    this.interval = this.interval || setInterval(refresh, 10_000)

    try {
      const res = await this.node.announceAsync(this.str2hex(val), port)
      this.services[val] = ['foo'] // let it be looked up
      await refresh()
  ...
  start (cb = noop) {
    ...
    this.createNode(
      (err, node) => {
        ...
        this.node = node
        this.node.announceAsync = promisify(this.node.announce)
